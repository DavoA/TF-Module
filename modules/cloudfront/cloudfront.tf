...
resource "aws_cloudfront_distribution" "my_distribution" {
  count = var.enable_cloudfront ? 1 : 0
  depends_on = [ aws_iam_role.cloudfront_s3_access_role ]
  origin {
    domain_name = var.domain
    origin_id   = var.origin
    s3_origin_config {
      origin_access_identity = aws_cloudfront_origin_access_identity.my_oai[0].cloudfront_access_identity_path
    }
  }
